...
class Smarty_Internal_TestInstall
{
    ...
    public static function testInstall(Smarty $smarty, &$errors = null)
    {
        ...
        if (!$_core_plugins_available) {
            $status = false;
            $message = "WARNING: Smarty's own smarty_inc/plugins is not available";
            if ($errors === null) {
                echo $message . ".\n";
        ...
        if (is_dir($source)) {
            ...
            if ($expectedSysplugins) {
                $status = false;
                $message = "FAILED: files missing from smarty_inc/sysplugins: " . join(', ', array_keys($expectedSysplugins));
                if ($errors === null) {
                    echo $message . ".\n";
                } else {
        ...
        if (is_dir($source)) {
            ...
            if ($expectedPlugins) {
                $status = false;
                $message = "FAILED: files missing from smarty_inc/plugins: " . join(', ', array_keys($expectedPlugins));
                if ($errors === null) {
                    echo $message . ".\n";
                } else {
